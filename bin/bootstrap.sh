...
mkdir -p "$CLIENTS_DIR"
mkdir -p "$DOTFILES/working/runtime"
_legacy_active_client="$DOTFILES/working/active-client.md"
_active_client="$DOTFILES/working/runtime/active-client.md"
if [[ -f "$_legacy_active_client" ]]; then
    if grep -qF "Auto-generated by detect-client.sh" "$_legacy_active_client" 2>/dev/null; then
        rm -f "$_legacy_active_client"
        green "  Removed legacy working/active-client.md runtime file"
    else
        yellow "  Legacy working/active-client.md is not auto-generated — move it manually if still needed"
    fi
fi
if [[ ! -f "$_active_client" ]]; then
    cat > "$_active_client" << 'ACTIVECLIENTEOF'
<!-- Auto-generated by detect-client.sh — no client context active -->
<!-- This file is regenerated on every cd() — do not edit manually -->
ACTIVECLIENTEOF
    green "  Created working/runtime/active-client.md placeholder"
else
    yellow "  working/runtime/active-client.md already exists — skipping"
fi
unset _legacy_active_client _active_client
