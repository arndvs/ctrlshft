...
mkdir -p "$CLAUDE_DIR"
mkdir -p "$DOTFILES/skills/local"
mkdir -p "$DOTFILES/instructions/local"

# Generate CLAUDE.md from base + local instruction references
cp "$DOTFILES/CLAUDE.base.md" "$DOTFILES/CLAUDE.md"
_local_instructions=()
if [[ -d "$DOTFILES/instructions/local" ]]; then
    for f in "$DOTFILES/instructions/local/"*.instructions.md; do
...
if [[ ${#_local_instructions[@]} -gt 0 ]]; then
    ...
    for f in "${_local_instructions[@]}"; do
        _fname=$(basename "$f")
        printf '@~/dotfiles/instructions/local/%s\n' "$_fname" >> "$DOTFILES/CLAUDE.md"
    done
