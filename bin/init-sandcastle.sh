...
if ! (cd .sandcastle/engine && pnpm install --frozen-lockfile --ignore-workspace); then
    red "    pnpm install failed in .sandcastle/engine/ — engine dependencies are required to run agents."
    red "    Fix the error above, then re-run: (cd .sandcastle/engine && pnpm install --frozen-lockfile --ignore-workspace)"
    exit 1
fi
