...
#
# Usage: ctrl init-sandcastle [--branch main] [--model claude-opus-4-6] [--pm npm] [--force]
#
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --help|-h)
            echo "Usage: ctrl init-sandcastle [--branch main] [--model claude-opus-4-6] [--pm npm] [--force]"
            exit 0
            ;;
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    if [[ -f "$LABELS_FILE" ]]; then
        if command -v node &>/dev/null; then
            node -e "
                const labels = JSON.parse(require('fs').readFileSync('$LABELS_FILE', 'utf8'));
...
echo ""
echo "  2. (Optional) Add repo secret: AGENT_PAT"
