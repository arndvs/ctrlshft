...
You are addressing reviewer feedback on PR #{{PR_NUMBER}} (branch `{{BRANCH}}`), round {{ROUND}}.
...
<output>
{
  "threadReplies": [
    ...
    },
    {
      ...
      "body": "I'd push back on this one — renaming would conflict with conventions elsewhere in this file. Happy to do it as a follow-up."
    }
  ],
  "newInlineComments": [
    {
  ...
  "topLevelComments": [
    {
      "body": "Round {{ROUND}}: Addressed 2 of 3 threads. Left the third for a separate PR — explained inline."
    }
  ]
}
</output>
...
The workflow will mark the run as blocked if both the diff and all reply arrays are empty.
