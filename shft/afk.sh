...
# ── RALPH loop (SHFT_ENGINE=ts) removed ──────────────────────────────────────
# The plan→implement→merge loop (main.ts) has been deprecated in favour of
# standalone CI-triggered workflows dispatched via .sandcastle/run.ts.
# See PRD #4 / issue #5 for context.
