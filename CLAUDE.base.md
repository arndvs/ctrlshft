...
- `better-auth` → `~/dotfiles/instructions/better-auth.instructions.md`

If `$ACTIVE_CONTEXTS` is not set, fall back to checking for file signatures (`next.config.*`, `composer.json`, `sanity.config.*`, `metro.config.*`, `better-auth` in `package.json`, `auth.ts` importing Better Auth, etc.) and load the matching instructions above.
...
`bootstrap.sh` auto-appends `@`-references only for auto-loaded `*.instructions.md` files found in `instructions/_local/`. Files with `auto-load: false` in their frontmatter are registered as task-triggered plain paths instead — read them only when the task matches their description. Conditional instructions intentionally avoid `@` syntax because Claude Code may treat `@` references as eager context. Run `bootstrap.sh` after adding new local instruction files.
