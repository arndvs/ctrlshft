...
Prefer the array-of-rules pattern for multiple validation constraints. It separates concerns more clearly than long method chains.

```typescript
// ✅ Preferred: Array of rules (multiple constraints)
defineField({
  name: 'title',
  type: 'string',
  validation: (rule) => [
    rule.required().error('Title is required'),
    rule.min(10).warning('Consider a longer title'),
    rule.max(70).warning('May be truncated in search results')
  ]
})

// ✅ Also valid: Single chain (simple cases)
defineField({
  name: 'title',
  type: 'string',
