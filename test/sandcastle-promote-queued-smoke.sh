#!/usr/bin/env bash
# test/sandcastle-promote-queued-smoke.sh — Verify Sandcastle promote-queued smoke harness behavior.
# Usage: bash test/sandcastle-promote-queued-smoke.sh
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SCRIPT="$ROOT/bin/smoke-sandcastle-promote-queued.sh"
TMP_ROOT="$ROOT/working/tmp/sandcastle-promote-queued-smoke-test"
...
make_repo() {
    ...
    cat > "$repo/.github/workflows/agent-promote-queued.yml" <<'YAML'
...
echo
echo "Sandcastle promote-queued smoke tests"
echo "════════════════════════════════════════════════"
...
# ── Dry-run test ──────────────────────────────────────────────────────────────
run_case "dry run previews promote-queued chain" pass "would:" \
    bash -c "cd '$repo' && DOTFILES='$ROOT' '$SCRIPT' --dry-run --repo owner/repo"

# ── Dry run shows pair count ──────────────────────────────────────────────────
run_case "dry run shows pair count" pass "2 pair" \
    bash -c "cd '$repo' && DOTFILES='$ROOT' '$SCRIPT' --dry-run --repo owner/repo --pairs 2"
...
# ── Minimum 2 pairs enforced ─────────────────────────────────────────────────
run_case "minimum 2 pairs enforced" fail "must be at least 2" \
    bash -c "cd '$repo' && DOTFILES='$ROOT' '$SCRIPT' --dry-run --repo owner/repo --pairs 1"

# ── Missing workflow fails ────────────────────────────────────────────────────
missing_wf_repo="$TMP_ROOT/missing-wf-repo"
make_repo "$missing_wf_repo"
rm -f "$missing_wf_repo/.github/workflows/agent-promote-queued.yml"
