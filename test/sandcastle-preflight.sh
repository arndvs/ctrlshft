...
run_case "malformed workflow reports SYNTAX" fail "SYNTAX" bash -c "cd '$malformed_workflow' && DOTFILES='$ROOT' ANTHROPIC_API_KEY=dummy CLAUDE_CODE_OAUTH_TOKEN=dummy AGENT_PAT=dummy '$CTRL' preflight-sandcastle --skip-drift --skip-engine --skip-github"
...
PY
run_case "missing workflow permissions reports PERMS" fail "PERMS" bash -c "cd '$missing_permissions' && DOTFILES='$ROOT' ANTHROPIC_API_KEY=dummy CLAUDE_CODE_OAUTH_TOKEN=dummy AGENT_PAT=dummy '$CTRL' preflight-sandcastle --skip-drift --skip-engine --skip-github"

healthy="$TMP_ROOT/healthy"
make_repo "$healthy"
seed_minimal_sandcastle "$healthy"
run_case "healthy minimal repo passes skipped external gates" pass "Sandcastle preflight passed" bash -c "cd '$healthy' && DOTFILES='$ROOT' ANTHROPIC_API_KEY=dummy CLAUDE_CODE_OAUTH_TOKEN=dummy AGENT_PAT=dummy '$CTRL' preflight-sandcastle --skip-drift --skip-engine --skip-github"
