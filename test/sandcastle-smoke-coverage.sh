#!/usr/bin/env bash
# test/sandcastle-smoke-coverage.sh — Verify every Sandcastle workflow template
# has smoke test coverage and the report aggregator inventory matches templates.
#
...
#   1. Every agent workflow template is listed in the report aggregator.
#   2. Every agent workflow template maps to at least one smoke script or report path.
...
# ── 1. Agent workflow templates ──────────────────────────────────────────────
# Product workflow source lives in shft/templates; dotfiles-private intentionally
# does not install root .github/workflows/*.yml actions.
agent_workflow_templates=()
for wf in "$ROOT"/shft/templates/workflows/agent-*.yml; do
    [[ -f "$wf" ]] || continue
    agent_workflow_templates+=("$(basename "$wf")")
done

if [[ ${#agent_workflow_templates[@]} -ge 10 ]]; then
    _record_pass "at least 10 agent workflow templates exist (${#agent_workflow_templates[@]})"
else
    _record_fail "at least 10 agent workflow templates exist" "found ${#agent_workflow_templates[@]}"
fi

# ── 2. Report aggregator inventory completeness ──────────────────────────────
# The report aggregator lists workflows by display name in SANDCASTLE_WORKFLOWS.
# Map each agent workflow template file to its display name and check presence.
...
if [[ ! -f "$report_script" ]]; then
    ...
    for wf_file in "${agent_workflow_templates[@]}"; do
...
for wf_file in "${agent_workflow_templates[@]}"; do
    ...
    # Passive coverage: the report aggregator queries all workflows by name.
    wf_path="$ROOT/shft/templates/workflows/$wf_file"
    display_name="$(grep -E '^name:' "$wf_path" | head -1 | sed 's/^name:[[:space:]]*"//;s/"$//' | sed "s/^name:[[:space:]]*//")"
    display_name="${display_name//\"/}"
...
if [[ -f "$matrix_doc" ]]; then
    ...
    # Verify matrix references every agent workflow template.
    matrix_missing=()
    for wf_file in "${agent_workflow_templates[@]}"; do
        wf_stem="${wf_file%.yml}"
    ...
    if [[ ${#matrix_missing[@]} -eq 0 ]]; then
        _record_pass "smoke matrix references all ${#agent_workflow_templates[@]} agent workflow templates"
    else
        _record_fail "smoke matrix references all agent workflow templates" "missing: ${matrix_missing[*]}"
    fi
else
    _record_fail "full smoke matrix doc exists" "$matrix_doc not found"
fi

# ── 5. Smoke test scripts exist for each smoke script ────────────────────────
