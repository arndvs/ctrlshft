...
The pre-commit hook runs the regular `test` script with `SKIP_SLOW_TESTS=1`, matching the `test:fast` behavior so commits stay under 10 seconds. The full suite (including the heavyweight `hooks-integration` tests) runs via `npm test` and in CI.
